...
export const SealManagement = ({ idKaryawan, onBack, showToast, onDownloadQR }: SealManagementProps) => {
  const [page, setPage] = useState<'list' | 'create' | 'detail'>('list');
  ...
  // ============================================
  // DOKUMEN ROWS MANAGEMENT
  // ============================================
  const tambahBaris = () => {
    setDokumenRows([...dokumenRows, {}]);
  };

  const hapusBaris = (index: number) => {
    setDokumenRows(dokumenRows.filter((_, i) => i !== index));
  };
  ...
  const handleUploadExcel = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (event) => {
      try {
        ...
        // Ambil kolom dari header Excel (kecuali nomor_dokumen)
        const headers = Object.keys(jsonData[0]).filter(h => h !== 'nomor_dokumen');
        if (!Object.keys(jsonData[0]).includes('nomor_dokumen')) {
          showToast('Kolom "nomor_dokumen" wajib ada di Excel', 'error');
          return;
        }

        // Update kolom custom dari header Excel
        setKolomCustom(headers);

        // Set rows dari data Excel
  ...
  const handleCreateBatch = async (e: React.FormEvent) => {
    ...
    if (!formNamaBatch || !formJenisDokumen || !formNamaOrg) {
      showToast('Nama batch, jenis dokumen, dan nama organisasi wajib diisi', 'error');
      return;
    }

    if (kolomCustom.length === 0) {
      showToast('Minimal 1 kolom custom harus ditambahkan', 'error');
      return;
    }
          ...
          {filteredBatches.map(batch => (
            <Card key={batch.id_batch} className="p-5 flex flex-col md:flex-row gap-4 items-start md:items-center group">
              ...
              <div className="flex items-center gap-2 flex-shrink-0">
                <Button
                  variant="secondary"
                  onClick={() => fetchBatchDetail(batch.id_batch)}
      ...
      <form onSubmit={handleCreateBatch} className="space-y-6">
        ...
        <Card className="p-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="md:col-span-2">
              ...
              <Input
                required
                placeholder="Contoh: Sertifikat Kompetensi Batch Jan 2025"
                value={formNamaBatch}
                onChange={e => setFormNamaBatch(e.target.value)}
              />
            </div>
            <div>
              ...
              <Input
                ...
                value={formJenisDokumen}
                onChange={e => setFormJenisDokumen(e.target.value)}
              />
            </div>
            <div>
              ...
              <Input
                required
                placeholder="Contoh: PT. NNI System"
                value={formNamaOrg}
                onChange={e => setFormNamaOrg(e.target.value)}
              />
            </div>
          </div>
        </Card>
        ...
        <Card className="p-8">
          ...
          <div className="flex flex-wrap gap-2 mb-4">
            <span className="px-3 py-1.5 rounded-xl bg-gray-100 text-gray-500 text-sm font-mono font-bold">
              nomor_dokumen
            </span>
        ...
        <Card className="p-8">
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="bg-gray-50">
                  ...
                  <th className="text-left px-4 py-3 text-xs font-bold text-gray-400 uppercase whitespace-nowrap">
                    nomor_dokumen *
                  </th>
                  {kolomCustom.map(kolom => (
                    <th key={kolom} className="text-left px-4 py-3 text-xs font-bold text-gray-400 uppercase whitespace-nowrap">
                      {kolom}
                    </th>
                  ))}
                  ...
                  <tr key={index} className="border-t border-gray-50 hover:bg-gray-50/50">
                    ...
                    <td className="px-2 py-2">
                      <input
                        ...
                        placeholder="SERT-001"
                        value={row.nomor_dokumen || ''}
                        onChange={e => updateBaris(index, 'nomor_dokumen', e.target.value)}
                      />
                    </td>
                    {kolomCustom.map(kolom => (
                      <td key={kolom} className="px-2 py-2">
                        <input
                          ...
                          placeholder={kolom}
                          value={row[kolom] || ''}
                          onChange={e => updateBaris(index, kolom, e.target.value)}
                        />
                      </td>
                    ))}
                    <td className="px-2 py-2">
          ...
          <button
            type="button"
            onClick={tambahBaris}
            className="mt-4 w-full py-3 rounded-2xl border-2 border-dashed border-gray-200 text-gray-400 hover:border-indigo-300 hover:text-indigo-500 transition-all text-sm font-semibold flex items-center justify-center gap-2"
          >
        ...
        <Button type="submit" className="w-full py-5 text-base" disabled={loading}>
          {loading ? (
            <span className="flex items-center justify-center gap-2">
              <Loader2 size={20} className="animate-spin" /> Membuat Batch Seal...
            </span>
          ) : (
            <span className="flex items-center justify-center gap-2">
              <Shield size={20} /> Generate Batch Seal
            </span>
          )}
        </Button>
      </form>
  ...
  const renderDetail = () => {
    ...
    return (
      <div className="space-y-6">
        ...
        {/* Tabel Dokumen */}
        <Card className="p-6">
          <h3 className="font-bold text-gray-900 mb-4 text-lg">Daftar Dokumen ({selectedBatch.documents?.length || 0})</h3>
    ...
    <div className="space-y-8">
      ...
      {page !== 'create' && (
        <div className="flex bg-white p-1.5 rounded-2xl border border-gray-100 shadow-sm w-fit">
          {[
            { id: 'list', label: 'Daftar Batch', icon: <Shield size={16} /> },
          ].map(item => (
            <button
              key={item.id}
              ...
              className={cn(
                ...
                page === item.id ? "bg-indigo-600 text-white shadow-lg shadow-indigo-100" : "text-gray-500 hover:bg-gray-50"
              )}
            >
